...
export default function SolarSystem() {
  ...
  return (
    <div className="solar-canvas-frame relative w-full h-[520px] sm:h-[620px] lg:h-[700px] flex items-center justify-center select-none overflow-hidden rounded-3xl border border-white/15 bg-slate-950/90 shadow-2xl transition-all duration-500">
      ...
      <div className="absolute top-4 left-4 right-4 z-20 flex flex-wrap items-center justify-between gap-3 pointer-events-none">
        ...
        <div className="flex flex-wrap items-center gap-1.5 p-1.5 rounded-2xl bg-slate-950/85 border border-white/10 backdrop-blur-md pointer-events-auto max-w-full overflow-x-auto shadow-xl">
          <button
          ...
          <button
            ...
            className={`px-3 py-1 rounded-xl text-xs font-mono font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
              selectedPlanet === 'earth'
                ...
                : 'text-cyan-400 hover:text-cyan-300 bg-cyan-950/40 border border-cyan-500/30'
            }`}
          >
        ...
        <div className="flex items-center gap-2 p-1.5 rounded-2xl bg-slate-950/85 border border-white/10 backdrop-blur-md pointer-events-auto shadow-xl">
          <button
      ...
      {selectedPlanet === 'earth' && (
        <div className="absolute bottom-4 left-4 right-4 z-30 p-4 rounded-3xl bg-slate-950/95 border border-cyan-500/40 backdrop-blur-2xl shadow-2xl flex flex-col md:flex-row items-center justify-between gap-4 animate-in fade-in slide-in-from-bottom duration-200">
          {/* Status Label */}
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-cyan-500/15 border border-cyan-500/30 flex items-center justify-center text-cyan-400">
              {dayNightPhase === 'day' || dayNightPhase === 'dawn' ? (
                <Sun className="w-5 h-5 text-amber-400 animate-spin" style={{ animationDuration: '10s' }} />
              ) : (
                <Moon className="w-5 h-5 text-cyan-300" />
              )}
            </div>
            <div>
              <p className="text-xs font-mono font-bold uppercase tracking-wider text-cyan-400 flex items-center gap-1.5">
              ...
              <p className="text-xs text-slate-400">
                Move Sun & Moon to transition website illumination step-by-step.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-80">
            <span className="text-xs font-mono text-slate-400 flex items-center gap-1">
              <Moon className="w-3.5 h-3.5 text-cyan-400" /> Night
            </span>
            ...
            <span className="text-xs font-mono text-amber-400 flex items-center gap-1">
              Day <Sun className="w-3.5 h-3.5 text-amber-400" />
            </span>
          </div>
          ...
          <button
            onClick={() => dispatch(setSelectedPlanet('all'))}
            className="px-4 py-2 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-mono text-xs font-semibold transition-colors cursor-pointer"
          >
